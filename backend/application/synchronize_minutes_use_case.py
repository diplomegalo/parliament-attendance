...
class SynchronizeMinutesUseCase:
    ...
    def _filter_sessions_by_business_rules(
        ...
        if definitive_sessions:
            ...
            existing_refs = self.minute_repo.find_existing_references(
                definitive_refs
            )
            ...
            self.logger.info(
                f"Found {len(unregistered_definitive)} "
                f"unregistered definitive sessions"
            )
        else:
            unregistered_definitive = []
    ...
    def _create_minutes_from_sessions(
        ...
        for session in sessions:
            try:
                ...
                self.logger.debug(
                    f"Created minute for session {session.reference}"
                )
                
            except Exception as e:
                self.logger.error(
                    f"Failed to create minute for "
                    f"session {session.reference}: {e}"
                )
                continue
